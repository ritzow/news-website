...
public class TempContent {
	...
	public static final String markdown = """
		...
		# Sandbox2D
		A 2D block building and destruction platformer game written in Java that uses OpenGL for graphics, OpenAL for
		audio, GLFW for cross-platform windowing and input, and LWJGL for bindings to the afformentioned libraries and APIs.
		
		# Repository Structure
		**/shared**: Contains code shared by client and server *(module ritzow.sandbox.shared)*.
		
		**/client**: Contains client-side code and resources including game assets and OpenGL shader code
		...
		**/serverlauncher**: JavaFX dedicated server GUI *(module ritzow.sandbox.serverlauncher)*.
		...
		```
		### TWL's PNGDecoder:
		Used for loading textures. Does not seem to be available directly anymore as the official
		website seems to be in use by scammers. A modified copy is included in the client module
		(and other libraries including slick-util also include a copy).
