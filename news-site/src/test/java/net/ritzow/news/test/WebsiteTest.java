...
public class WebsiteTest {
	@Test
	void run(@TempDir(cleanup = CleanupMode.ALWAYS) Path temp) throws Exception {
...
//			CertificateAuthority.ipAddressName(InetAddress.getByName("::1")));
		...
		var server = NewsSite.builder()
			.setRequireSni(false)
			.setKeyStore(Certs.selfSigned(new GeneralNames(addresses.stream().map(addr -> new GeneralName(GeneralName.iPAddress, new DEROctetString(addr.getAddress())))
				.toArray(GeneralName[]::new)), organization, password.toCharArray(), random))
			.setKeyStorePassword(password)
			.setPeers(addresses.stream().map(InetAddress::getHostAddress).collect(Collectors.toSet()))
			.setBind(addresses.toArray(InetAddress[]::new))
			.setDatabaseDir(temp)
			.start();
